...
void SpaceshipScene::Initialize(IDirect3DDevice9* d3dDevice)
{
    ...
    for (int i = 0; i < *(&collectible + 1) - collectible; i++)
    {
        ...
        collectible[i]->Init(d3dDevice, rand() % (WINDOW_WIDTH - COLLECTIBLE_SPRITEWIDTH),
                             rand() % (WINDOW_HEIGHT - COLLECTIBLE_SPRITEHEIGHT));
    }
...
void SpaceshipScene::Update(int framesToUpdate)
{
    ...
    else
    {
        for (int i = 0; i < framesToUpdate; i++)
        {
            ...
            if (Physics::CircleColDetection(player1->GetSpaceship()->GetSpriteWidth() / 2,
                                            player2->GetSpaceship()->GetSpriteWidth() / 2,
                                            player1->GetSpaceship()->GetPosition() + player1->GetSpaceship()->
                                            GetSpriteCenter(),
                                            player2->GetSpaceship()->GetPosition() + player2->GetSpaceship()->
                                            GetSpriteCenter()))
            {
            ...
            for (int i = 0; i < *(&collectible + 1) - collectible; i++)
            {
                ...
                {
                    if (Physics::CircleColDetection(player1->GetSpaceship()->GetSpriteWidth() / 2,
                                                    collectible[i]->GetCollectible()->GetSpriteWidth() / 2,
                                                    player1->GetSpaceship()->GetPosition() + player1->GetSpaceship()->
                                                    GetSpriteCenter(),
                                                    collectible[i]->GetCollectible()->GetPosition() + collectible[i]->
                                                    GetCollectible()->GetSpriteCenter())
                        || Physics::CircleColDetection(player2->GetSpaceship()->GetSpriteWidth() / 2,
                                                       collectible[i]->GetCollectible()->GetSpriteWidth() / 2,
                                                       player2->GetSpaceship()->GetPosition() + player2->GetSpaceship()
                                                       ->GetSpriteCenter(),
                                                       collectible[i]->GetCollectible()->GetPosition() + collectible[i]
...
void SpaceshipScene::Render(LPD3DXSPRITE spriteBrush)
{
...
void SpaceshipScene::RenderLine(LPD3DXSPRITE spriteBrush)
{
